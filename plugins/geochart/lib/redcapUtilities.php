 ...
 * $Date:: 2012-10-13 12:04:33 #$
 * $URL: https://srcvault.icts.uiowa.edu/repos/REDCap/REDCap/tags/geochart-1.0/lib/redcapUtilities.php $
 */
...
function GetAllFieldNames( $projectId = PROJECT_ID )
{
   ...
   {
      die( "Error 261: Could not execute SQL:
            <pre>$sql</pre> <br />" .
            mysql_error() );
   }
...
function GetGeoFieldNames( $projectId = PROJECT_ID )
{
   ...
   if ( ! $result )  // sql failed
   {
      die( "Error 297: Could not execute SQL:
            <pre>$sql</pre> <br />" .
            mysql_error() );
   }
 ...
 *                      @endcode
 */
function GetMultipleFieldValues( $projectId, $fieldNameArray, $filterNameHash = array() )
{
   // intialize variables
   $count = 0;
...
ORDER BY %s.record", $aliasArray[0] );  // sort by the "study_id"
   ...
   if ( ! $result )  // sql failed
   {
      die( "Error 144: Could not execute SQL:
            <pre>$sql</pre> <br />" .
            mysql_error() );
   }
...
function GetFormName( $projectId, $fieldName )
{
   ...
   if ( ! $result )  // sql failed
   {
      die( "Error 183: Could not execute SQL:
            <pre>$sql</pre> <br />" .
            mysql_error() );
   }
...
function GetDataEventId( $projectId, $fieldName )
{
   ...
   if ( ! $result )  // sql failed
   {
      die( "Error 221: Could not execute SQL:
            <pre>$sql</pre> <br />" .
            mysql_error() );
   }
